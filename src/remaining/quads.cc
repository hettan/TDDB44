...
sym_index ast_integer::generate_quads(quad_list &q)
{
    ...
    q += new quadruple(q_iload, value, NULL_SYM, addr);
    return addr;
}
...
sym_index ast_real::generate_quads(quad_list &q)
{
    ...
    q += new quadruple(q_rload, sym_tab->ieee(value), NULL_SYM, addr);
    return addr;
}
...
sym_index do_bin_single(quad_list &q, quad_op_type op_int, quad_op_type op_real, ast_expression *node)
{
  sym_index addr;
  if(node->type == integer_type) {
    sym_index sym_p = node->generate_quads(q);
    addr = sym_tab->gen_temp_var(node->type);
    q += new quadruple(op_int, sym_p, NULL_SYM, addr);
  }
  else {
    sym_index sym_p = node->generate_quads(q);
    addr = sym_tab->gen_temp_var(node->type);
    q += new quadruple(op_real, sym_p, NULL_SYM, addr);
  }
  return addr;
}

sym_index do_binop(quad_list &q, quad_op_type op_int, quad_op_type op_real, ast_binaryoperation *node)
{
  sym_index addr;
  if(node->type == integer_type) {
    sym_index sym_p_left = node->left->generate_quads(q);
    sym_index sym_p_right = node->right->generate_quads(q);
    addr = sym_tab->gen_temp_var(node->type);
    q += new quadruple(op_int, sym_p_left, sym_p_right, addr);
  }
  else {
    sym_index sym_p_left = node->left->generate_quads(q);
    sym_index sym_p_right = node->right->generate_quads(q);
    addr = sym_tab->gen_temp_var(node->type);
    q += new quadruple(op_real, sym_p_left, sym_p_right, addr);
  }
  return addr;
}
...
sym_index do_binrel(quad_list &q, quad_op_type op_int, quad_op_type op_real, ast_binaryrelation *node)
{
  ...
  if(node->left->type == integer_type && node->right->type == integer_type) {
    ...
    fatal("should not be able to bee here!");
  }
  return addr;
}
...
sym_index ast_not::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_bin_single(q, q_inot, q_inot, expr);
}
...
sym_index ast_uminus::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_bin_single(q, q_iuminus, q_ruminus, expr);
}
...
sym_index ast_add::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binop(q, q_iplus, q_rplus, this); 
}
...
sym_index ast_sub::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binop(q, q_iminus, q_rminus, this); 
}
...
sym_index ast_mult::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binop(q, q_imult, q_rmult, this); 
}
...
sym_index ast_divide::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binop(q, q_idivide, q_rdivide, this); 
}
...
sym_index ast_idiv::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binop(q, q_idivide, q_idivide, this); 
}
...
sym_index ast_mod::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binop(q, q_imod, q_imod, this); 
}
...
sym_index ast_or::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binop(q, q_ior, q_ior, this); 
}
...
sym_index ast_and::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binop(q, q_iand, q_iand, this); 
}
...
sym_index ast_equal::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binrel(q, q_ieq, q_req, this); 
}
...
sym_index ast_notequal::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binrel(q, q_ine, q_rne, this); 
}
...
sym_index ast_lessthan::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binrel(q, q_ilt, q_rlt, this); 
}
...
sym_index ast_greaterthan::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binrel(q, q_igt, q_rgt, this); 
}
...
void generate_param_list(quad_list &q, ast_expr_list* param, int *nr_params)
{
  if(param != NULL) {
    (*nr_params)++;
    q += new quadruple(q_param, param->last_expr->generate_quads(q), NULL_SYM, NULL_SYM);
    generate_param_list(q, param->preceding, nr_params);
  }
}
...
sym_index ast_procedurecall::generate_quads(quad_list &q)
{
    ...
    q += new quadruple(q_call, id->sym_p, param_counter, NULL_SYM);
    return NULL_SYM;
}
...
void ast_elsif::generate_quads_and_jump(quad_list &q, int label)
{
    USE_Q;
    /* Your code here */
    sym_index sym_p = condition->generate_quads(q);
    int next_label = sym_tab->get_next_label();
    q += new quadruple(q_jmpf, next_label, sym_p, NULL_SYM);
...
sym_index ast_if::generate_quads(quad_list &q)
{
    ...
    /* Your code here */
    sym_index sym_p = condition->generate_quads(q);
    int if_false_label = sym_tab->get_next_label();
    int end_label = sym_tab->get_next_label();

    q += new quadruple(q_jmpf, if_false_label, sym_p, NULL_SYM);
    if(body != NULL) {
      body->generate_quads(q);
    ...
    if(elsif_list != NULL) {      
      q += new quadruple(q_labl, if_false_label, NULL_SYM, NULL_SYM);
...
sym_index ast_indexed::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    sym_index sym_p_left = id->generate_quads(q);
    sym_index sym_p_right = index->generate_quads(q);
    sym_index addr = sym_tab->gen_temp_var(type);
    if(type == integer_type) {
      q += new quadruple(q_irindex, sym_p_left, sym_p_right , addr);
    }
    else {
      q += new quadruple(q_rrindex, sym_p_left, sym_p_right, addr);
    }
    return addr;
}
